...
export const createSloganLetterStyles =  (index:number,isInit:boolean,isStart:boolean,isSpace:boolean) => {

    const letterAnimation =  createLetterAnimation(index,isInit,isStart,isSpace);
    return css({
        color: colors.LIGHTORANGE,
        flex: '1 1 auto',
        marginRight: isSpace ? isInit ? SPACE_LETTER_MID_MARGIN_RIGHT : SPACE_LETTER_LEFT_MARGIN_RIGHT : '0px',
        animation : letterAnimation,
    });
}

const createLetterAnimation = (index:number, isInit:boolean, isStart:boolean, isSpace:boolean) => {
    if(isInit){
